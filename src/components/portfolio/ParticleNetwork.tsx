...
const ParticleNetwork = ({
  ...
  useEffect(() => {
    ...
    // Create particles — more, bigger, better spread
    const { w, h } = getSize();
    const area = w * h;
    ...
    // Always listen on window — fires everywhere, coords are in viewport
    // space which matches the fixed-inset-0 canvas perfectly. Canvas stays
    // pointer-events-none so nothing is ever blocked.
    const mouseTarget: EventTarget = mode === "contained" ? canvas : window;

    mouseTarget.addEventListener("mousemove", handleMouseMove as EventListener);
    mouseTarget.addEventListener("mouseleave", handleMouseLeave as EventListener);
    ...
    const animate = () => {
      ...
      // Update positions — always moving autonomously
      for (const p of particles) {
        // Gradually return to base velocity so particles always drift
        ...
        // Speed cap to prevent weird condensed shapes
        const maxSpeed = 1.5;
    ...
    return () => {
      ...
      mouseTarget.removeEventListener("mouseleave", handleMouseLeave as EventListener);
    };
